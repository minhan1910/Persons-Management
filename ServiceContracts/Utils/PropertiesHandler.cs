...
namespace ServiceContracts.Utils
{
                                                     ...
                                                     where T2: class
    {
                            ...
                            if (v1 is not null)
                            {
                                ...
                                if (IsNullable(p1PropInfo.PropertyType))
                                {
                                    ...
                                    if (typeFromNullableTypeOfP1PropInfo == typeof(GenderOptions))
                                    {
                                        ...
                                        p2PropInfo.SetValue(dest, v1.ToString());
                                    }

                                    // Convert string to Enum
                                    if (typeFromNullableTypeOfP1PropInfo == typeof(string))
                                    {
                                        // p2PropInfo can not be nullable because if statement catch
                                                        ...
                                                        T descOfProp2)
        {
            ...
            {
                // Enum.GetName is affecting the performance because it boxing into object type.
                //prop2.SetValue(descOfProp2, Enum.GetName(prop1.PropertyType, valueofProp1));
                prop2.SetValue(descOfProp2, valueofProp1.ToString());
            }

            // convert string to Enum
            if (prop1.PropertyType == typeof(string))
            {
                // case for prop2 is nullable
